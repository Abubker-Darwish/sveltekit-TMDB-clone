<script context="module">
	...
	export const load = async ({ fetch }) => {
		let url = `https://api.themoviedb.org/3/tv/on_the_air?api_key=${variables?.basePath}`;
		const res = await fetch(url);
		const on_the_air = await res.json();

		return {
			props: { list: on_the_air?.results }
		};
	};
</script>
...
<div class="bg-white min-h-screen py-8">
	<div class="container">
		<h1 class="text-2xl font-semibold">Currently Airing TV Shows</h1>
		<div class="w-0 h-4" />
		<div class="list-box">
		...
		<div class="w-0 h-4" />
		<button class="load_more_btn" on:click={() => onTV.loadMore($onTV.page + 1)}>
			{$onTV.fetching ? 'loading ...' : 'load more'}
		</button>
	</div>
</div>

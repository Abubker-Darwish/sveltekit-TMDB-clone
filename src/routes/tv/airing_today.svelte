<script context="module">
	...
	export const load = async ({ fetch }) => {
		let url = `https://api.themoviedb.org/3/tv/airing_today?api_key=${variables?.basePath}`;
		const res = await fetch(url);
		const now_playing = await res.json();

		return {
			props: { list: now_playing?.results }
		};
	};
</script>
...
<script lang="ts">
	...
	import { airingTodayTV } from '$stores/tv/airing_today';
	import { goto } from '$app/navigation';

	export let list = [];

	onMount(() => {
		airingTodayTV.setMovies(list);
	});
</script>

<div class="bg-white min-h-screen py-8">
	<div class="container">
		<h1 class="text-2xl font-semibold">TV Shows Airing Today</h1>
		<div class="w-0 h-4" />
		<div class="list-box">
			{#each $airingTodayTV.list as item}

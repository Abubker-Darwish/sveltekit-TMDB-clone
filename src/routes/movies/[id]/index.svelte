<script context="module">
	...
	export const load = async ({ fetch, params }) => {
		...
		console.log('id', id);
		const path = `https://api.themoviedb.org/3/movie/${id}?api_key=${variables?.basePath}`;
...
<script lang="ts">
	...
	//   runtime: number;
	// }
	export let movie: any = { title: '', runtime: 0 };
...
<div class="bg-white min-h-screen">
	<div
		...
		style="background-image: url({coverImgUrl}{movie.backdrop_path});"
	>
		<div class="absolute h-full z-[1] w-full p-8 flex items-center ">
			<div class="flex items-center gap-5">
				<img src="{baseImgUrl}{movie.poster_path}" alt="" class="w-[300px] h-[450px] rounded-lg" />
				...
				<div class="flex-1 items-center place-self-stretch">
					...
					<div class="flex items-center gap-2 mt-1 text-white text-base">
						<span>
							{format(new Date(movie.release_date), 'dd/MM/yyyy')} (AS)
						</span>
						...
						<span>
							{getTime(movie.runtime).hours}h {getTime(movie.runtime).minutes}m
						</span>
					</div>
					<div class="h-10 w-0" />
					...
					<div class="ml-3 flex">
						<div class="w-fit scale-125">
							<CardProgressBar percentage={movie.vote_average * 10} />
						</div>
					</div>
	
					<div class="mt-4">
						<h2 class="text-white text-xl font-medium">Overview</h2>
						<p  class="text-white/75 text-base font-medium">{movie.overview}</p>
					</div>
				</div>
			</div>
		</div>
